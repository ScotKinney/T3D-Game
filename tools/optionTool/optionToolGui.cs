   ...
   for ( %i = 1; %i < 7; %i++ )
   {
      ...
      {   
         ...
         if ( "/" $= getSubStr(%texFile, 0, 1) )
            %texFile = "art/shapes/actors" @ %texFile;
...
function OptionTool::doUpdateQuery(%this, %newOpt)
{
   ...
   %fullQuery = "Update AVOptions Set " @ %queryStr @ " Where ID='" @ %this.curItemID @ "'";
...
function OptionTool::makeQSGeneralTab(%this, %itemDB, %queryStr)
{
   ...
   %testVal = strreplace(%testVal, "art/shapes", "");
...
function OptionTool::makeQSMeshTab(%this, %itemDB, %queryStr)
{
   ...
   for ( %i = 1; %i < 7; %i++ )
   {
      ...
      %testVal = strreplace(%testVal, "art/shapes/actors", "");
...
function OptionTool::texFromMat(%this, %matName)
{
   ...
   %texFile = strreplace(%texFile, "art/shapes/actors", "");
   return %texFile;
}
