
singleton Material(svspartastairs1_wall1)
{
   mapTo = "wall1";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall1.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall1N.dds";
   useAnisotropic[0] = "1";
   materialTag0 = "Miscellaneous";
};

singleton Material(svspartastairs1_wood3)
{
   mapTo = "wood3";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawood3.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawood3N.dds";
   useAnisotropic[0] = "1";
   materialTag0 = "Miscellaneous";
};

singleton Material(svspartastairs1_wood3)
{
   mapTo = "wood3";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawood3.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawood3N.dds";
};

singleton Material(svspartastairs1_floor1)
{
   mapTo = "floor1";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartfloor1.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartfloor1N.dds";
};

singleton Material(svspartawall1_wall5)
{
   mapTo = "wall5";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall5.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall5N.dds";
   useAnisotropic[0] = "1";
   materialTag0 = "Miscellaneous";
};

singleton Material(svspartawall1_wall4)
{
   mapTo = "wall4";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall4.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall4N.dds";
};

singleton Material(svspartawindow1_wall3)
{
   mapTo = "wall3";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall3.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall3N.dds";
   useAnisotropic[0] = "1";
};

singleton Material(svspartawindow2_wood1)
{
   mapTo = "wood1";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartatrim1.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartatrim1N.dds";
   materialTag0 = "Miscellaneous";
};

singleton Material(svspartawindow3_floor2)
{
   mapTo = "floor2";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartfloor2.dds";
   specularPower[0] = "20";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartfloor2N.dds";
   useAnisotropic[0] = "1";
};

singleton Material(svspartafloor1_floor1)
{
   mapTo = "floor1";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartfloor1";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartfloor1N.dds";
   useAnisotropic[0] = "1";
   materialTag0 = "Miscellaneous";
};

singleton Material(svspartafloor1_wood2)
{
   mapTo = "wood2";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawood2.dds";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawood2N.dds";
   materialTag0 = "Miscellaneous";
   useAnisotropic[0] = "1";
};

singleton Material(svspartafloor1_wall4)
{
   mapTo = "wall4";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall4";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall4N.dds";
   materialTag0 = "Miscellaneous";
   planarReflection = "0";
   dynamicCubemap = "0";
   useAnisotropic[0] = "1";
};

singleton Material(svspartafloor1_wall4)
{
   mapTo = "wall4";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall4";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall4N.dds";
   materialTag0 = "Miscellaneous";
   planarReflection = "0";
   dynamicCubemap = "0";
};

singleton Material(svspartaroof1_roof)
{
   mapTo = "roof";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartanroof1";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartanroof1N.dds";
};

singleton Material(svspartadoorway3_wall2)
{
   mapTo = "wall2";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall2.dds";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall2N.dds";
   useAnisotropic[0] = "1";
};

singleton Material(svspartadoorway3_iconic3)
{
   mapTo = "iconic3";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/sv-ionic3.dds";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/sv-ionic3N.dds";
   useAnisotropic[0] = "1";
};

singleton Material(svspartadoorway3_iconic3)
{
   mapTo = "iconic3";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/sv-ionic3.dds";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/sv-ionic3N.dds";
};

singleton Material(svspartadoor1_door)
{
   mapTo = "door";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartandoor1";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartandoor1N.dds";
   useAnisotropic[0] = "1";
};

singleton Material(svspartadoor1_default2)
{
   mapTo = "default2";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartandoor1.dds";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartandoor1N.dds";
   useAnisotropic[0] = "1";
   materialTag0 = "Miscellaneous";
};

singleton Material(svspartadoorway5_wal1l)
{
   mapTo = "wal1l";
   diffuseMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall1";
   specular[0] = "0 0 0 1";
   specularPower[0] = "128";
   translucentBlendOp = "None";
   normalMap[0] = "art/Packs/Buildings/SP_Sparta/svspartawall1N.dds";
   useAnisotropic[0] = "1";
};

singleton Material(svspartadoorway5_trim2)
{
   mapTo = "trim2";
   diffuseMap[0] = "svspartatrim1";
   specularPower[0] = "128";
   translucentBlendOp = "None";
};

singleton Material(svspartawindow4_trim1)
{
   mapTo = "trim1";
   diffuseMap[0] = "svspartatrim1";
   specularPower[0] = "128";
   translucentBlendOp = "None";
};
